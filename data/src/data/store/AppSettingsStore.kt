...
class AppSettingsStore(externalMmkv: MMKV) : MMKVPreference(externalMmkv = externalMmkv) {
    ...
    val themeMode by enumFlow(ThemeMode.Dark)
    val appLanguage by enumFlow(AppLanguage.System)
    val colorTheme by enumFlow(AppColorTheme.Custom)
    val monetStyle by enumFlow(MonetStyle.Monochrome)
    val monetContrast by enumFlow(MonetContrast.Medium)
    val monetColorIntensity by floatFlow(0.18f)
    val themeAccentColorArgb by longFlow(0xFFB8B8B8L)
    val acgWallpaperSeedColorArgb by longFlow(0xFFB8B8B8L)
    ...
    val acgMainUiEnabled by boolFlow(true)
    ...
    val acgHomeQuote by strFlow("黑夜不是终点, 只是让线路更安静。")
    val acgHomeQuoteAuthor by strFlow("YumeBox Study")
